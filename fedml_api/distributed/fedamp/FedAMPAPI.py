...
from .FedAMPAggregator import FedAMPAggregator
from .FedAMPTrainer import FedAMPTrainer
from .FedAMPClientManager import FedAMPClientManager
from .FedAMPServerManager import FedAMPServerManager
...
def FedML_FedAMP_distributed(process_id, worker_number, device, comm, model, train_data_num, train_data_global, test_data_global,
...
def init_server(args, device, comm, rank, size, model, train_data_num, train_data_global, test_data_global,
    ...
    # aggregator
    worker_num = size - 1
    aggregator = FedAMPAggregator(train_data_global, test_data_global, train_data_num,
    ...
    if preprocessed_sampling_lists is None :
        server_manager = FedAMPServerManager(args, aggregator, comm, rank, size, backend)
    else:
        server_manager = FedAMPServerManager(args, aggregator, comm, rank, size, backend,
...
def init_client(args, device, comm, process_id, size, model, train_data_num, train_data_local_num_dict,
    ...
    backend = args.backend
    trainer = FedAMPTrainer(client_index, train_data_local_dict, train_data_local_num_dict, test_data_local_dict,
                            train_data_num, device, args, model_trainer)
    client_manager = FedAMPClientManager(args, trainer, comm, process_id, size,backend)
    client_manager.run()